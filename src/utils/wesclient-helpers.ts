 ...
 */
export function filterEventsByResource(events: EventInfo[], resourceId: Uint8Array): EventInfo[] {
  const resourceIdStr = Array.from(resourceId)
    .map((b) => b.toString(16).padStart(2, "0"))
    .join("");
 ...
 */
export function filterEventsByName(events: EventInfo[], eventName: string): EventInfo[] {
 ...
 * 检查资源是否存在
 */
export async function resourceExists(client: WESClient, resourceId: Uint8Array): Promise<boolean> {
  try {
 ...
 * 检查交易是否存在
 */
export async function transactionExists(client: WESClient, txId: string): Promise<boolean> {
  try {
