...
export class MarketService {
  ...
  async swapAMM(request: SwapRequest, wallet?: Wallet): Promise<SwapResult> {
    ...
    const swapParams: any = {
      ...
      amountIn:
        typeof request.amountIn === "bigint"
          ? request.amountIn.toString()
          : request.amountIn.toString(),
      amountOutMin:
        typeof request.amountOutMin === "bigint"
          ? request.amountOutMin.toString()
          : request.amountOutMin.toString(),
    };
  ...
  private validateSwapRequest(request: SwapRequest): void {
    ...
    // 3. 验证金额
    const amountIn =
      typeof request.amountIn === "bigint" ? request.amountIn : BigInt(request.amountIn);
  ...
  async addLiquidity(request: AddLiquidityRequest, wallet?: Wallet): Promise<AddLiquidityResult> {
    ...
    const addLiquidityParams: any = {
      ...
      amountA:
        typeof request.amountA === "bigint"
          ? request.amountA.toString()
          : request.amountA.toString(),
      amountB:
        typeof request.amountB === "bigint"
          ? request.amountB.toString()
          : request.amountB.toString(),
    };
  ...
  async removeLiquidity(
    ...
    const removeLiquidityParams: any = {
      ...
      amount:
        typeof request.amount === "bigint" ? request.amount.toString() : request.amount.toString(),
    };
    ...
    try {
      ...
      if (txResult && typeof txResult === "object") {
        ...
        for (const output of userOutputs) {
          ...
          if (outputAmount) {
            const amount =
              typeof outputAmount === "string" ? BigInt(outputAmount) : BigInt(outputAmount);
            amounts.push(amount);
          }
        }
  ...
  async createVesting(
    ...
    const amount = typeof request.amount === "bigint" ? request.amount : BigInt(request.amount);
    const startTime =
      typeof request.startTime === "bigint" ? request.startTime : BigInt(request.startTime);
    const duration =
      typeof request.duration === "bigint" ? request.duration : BigInt(request.duration);
      ...
      if (txResult && typeof txResult === "object") {
        ...
        for (const output of outputs) {
          ...
          if (outputType === "asset") {
            ...
            if (
              this.addressesEqual(ownerBytes, request.buyer) ||
