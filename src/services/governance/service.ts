...
export class GovernanceService {
    ...
    try {
      ...
      if (txResult && typeof txResult === 'object') {
        ...
        if (stateOutputs.length > 0) {
          ...
          if (stateID) {
            proposalID = typeof stateID === 'string' 
              ? (stateID.startsWith('0x') ? stateID.slice(2) : stateID)
              : bytesToHex(stateID);
          } else if (outpoint) {
            proposalID = outpoint;
          }
  ...
  async vote(request: VoteRequest, wallet?: Wallet): Promise<VoteResult> {
    ...
    // 4. 在 SDK 层构建交易草稿
    const voteWeight = typeof request.voteWeight === 'bigint' ? request.voteWeight : BigInt(request.voteWeight);
