...
export async function buildBatchTransferTransaction(
  ...
  if (totalSelectedAmount < totalAmount) {
    throw new Error(`Insufficient balance: need ${totalAmount.toString()}, have ${totalSelectedAmount.toString()}`);
  }

  // 6. 构建交易草稿（批量转账）
  const draft = buildBatchTransactionDraft(
    fromAddress,
    transfers,
    tokenID,
    selectedUTXOs
...
function buildBatchTransactionDraft(
  ...
  for (const transfer of transfers) {
    ...
    const amount = typeof transfer.amount === 'bigint' ? transfer.amount.toString() : transfer.amount.toString();
