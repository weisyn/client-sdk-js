...
async function queryResourceUTXOByOutPoint(
  ...
  if (!utxo) {
    throw new Error(
      `Resource UTXO not found: ${txId}:${outputIndex}. The UTXO may have been spent or the resource ID is incorrect.`
    );
  }
...
export async function buildTransferOwnershipTx(
  ...
  if (!txId || isNaN(outputIndex)) {
    throw new Error(
      `Invalid resourceId format: ${intent.resourceId}. Expected format: txId:outputIndex`
    );
  }
  ...
  try {
    ...
    if (errorMsg.includes("not found") || errorMsg.includes("NOT_FOUND")) {
      throw new Error(
        `Resource UTXO not found or already spent: ${intent.resourceId}. The resource may have been transferred or consumed.`
      );
    }
  ...
  if (!utxo || !utxo.output) {
    throw new Error(
      `Resource UTXO not found: ${intent.resourceId}. The UTXO may have been spent or the resource ID is incorrect.`
    );
  }
  ...
  if (intent.newOwnerAddress.startsWith("0x")) {
    ...
    try {
      ...
      throw new Error(
        `Invalid address format: ${intent.newOwnerAddress}: ${e instanceof Error ? e.message : String(e)}`
      );
    }
  }
...
export async function buildUpdateCollaboratorsTx(
            ...
            typeof key === "string"
              ...
              : key.value
                ? typeof key.value === "string"
                  ? key.value.startsWith("0x")
                    ? key.value.slice(2)
                    : key.value
                  : bytesToHex(key.value).slice(2)
                : "",
  ...
  if (intent.requiredSignatures > allKeys.length) {
    throw new Error(
      `requiredSignatures (${intent.requiredSignatures}) cannot exceed number of authorized keys (${allKeys.length})`
    );
  }
...
export async function buildGrantDelegationTx(
  ...
  if (currentLockingConditions.length === 0) {
    throw new Error(
      `No locking conditions found for resource ${intent.resourceId}. Cannot grant delegation.`
    );
  }
      ...
      if (keys.length > 0) {
        ...
        originalOwnerHex =
          typeof firstKey === "string"
            ? firstKey.startsWith("0x")
              ? firstKey
              : "0x" + firstKey
            : firstKey.value
              ? typeof firstKey.value === "string"
                ? firstKey.value.startsWith("0x")
                  ? firstKey.value
                  : "0x" + firstKey.value
                : bytesToHex(firstKey.value)
              : undefined;
        if (originalOwnerHex) {
          break;
        }
  ...
  if (!originalOwnerHex) {
    throw new Error(
      `Cannot determine original owner from current locking conditions for resource ${intent.resourceId}. The resource may have an unsupported locking condition type.`
    );
  }
  ...
  if (intent.delegateAddress.startsWith("0x")) {
    ...
    try {
      ...
      throw new Error(
        `Invalid delegate address format: ${intent.delegateAddress}: ${e instanceof Error ? e.message : String(e)}`
      );
    }
  }
  ...
  if (invalidOps.length > 0) {
    throw new Error(
      `Invalid authorized operations: ${invalidOps.join(", ")}. Valid operations: ${validOperations.join(", ")}`
    );
  }
  ...
  const delegationLock = {
    delegation_lock: {
      ...
      max_value_per_operation: intent.maxValuePerOperation
        ? BigInt(intent.maxValuePerOperation).toString()
        : "0",
    ...
    if (unlockHeight < 1000000) {
      ...
      try {
        ...
        if (typeof currentHeightResult === "string") {
          const cleanHex = currentHeightResult.startsWith("0x")
            ? currentHeightResult.slice(2)
            : currentHeightResult;
