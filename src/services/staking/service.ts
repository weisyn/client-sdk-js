...
export class StakingService {
  ...
  private validateStakeRequest(request: StakeRequest): void {
    ...
    if (request.lockBlocks !== undefined) {
      const lockBlocks =
        typeof request.lockBlocks === "bigint" ? request.lockBlocks : BigInt(request.lockBlocks);
      if (lockBlocks <= BigInt(0)) {
  ...
  private validateClaimRewardRequest(request: ClaimRewardRequest): void {
    ...
    // 2. 验证至少提供一个ID
    if (
      (!request.stakeId || request.stakeId.length === 0) &&
  ...
  async slash(request: SlashRequest, wallet?: Wallet): Promise<SlashResult> {
    ...
    throw new Error(
      "Slash not implemented: requires governance rules or slash contract (architecture reserved, business logic undefined)"
    );
  }
